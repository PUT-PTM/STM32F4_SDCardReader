...
/**
	...
	\author  	Mark McLean / STM

	\brief		Low level SD card access functions.

	\details	MDK-ARM, Target Processor: 	STM32F103 and similar
	
	\details	LINT 9/08/12
...
#define SDIO_TRANSFER_CLK_DIV              ((uint8_t)0x0)   //ustawione na 0
...
/*  SDIO_CmdInitStructure.SDIO_Argument = 0x00;
  ...
  if (errorstatus == SD_OK)
  {
    ...
    if( SD_HIGH_CAPACITY == ( response & SD_HIGH_CAPACITY ))
    {
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  if ((CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1) || (CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0))
  {
    ...
    cardinfo->CardCapacity *= (1UL << (cardinfo->SD_csd.DeviceSizeMul + 2));
    cardinfo->CardBlockSize = 1UL << (cardinfo->SD_csd.RdBlockLen);
...
SD_Error SD_WriteBlock(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize)
{
  ...
  uint32_t timeout, bytestransferred = 0;
  uint32_t cardstatus, count = 0, restwords = 0;
...
* Return         : None
*******************************************************************************/
static uint8_t convert_from_bytes_to_power_of_two(uint16_t NumOfBytes)
{
  uint8_t count = 0;
...
/*******************************************************************************
* Function Name  : SDIO_Configuration
...
static void DMA_TxConfiguration(uint32_t *BufferSRC, uint32_t BufferSize)
{
 ...
 | DMA_FLAG_DMEIF3 |  DMA_FLAG_TEIF3 | DMA_FLAG_HTIF3 |  DMA_FLAG_TCIF3);
  //na stm32f4 SDIO jest na DMA2 Channel4 Stream3 i Stream6
  ...
  /* ELM FS specifically says that the data buffer may not be word aligned	*/
